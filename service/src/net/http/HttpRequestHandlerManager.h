...
public:
    void registerHandler(http::verb method, std::string_view path, HttpRequestHandlerFn handler) {
        _handlers.emplace(Request{method, path.data()}, handler);
    }

    void registerHandler(http::verb method, std::string_view path, HttpRequestHandler::Ptr handler) {
